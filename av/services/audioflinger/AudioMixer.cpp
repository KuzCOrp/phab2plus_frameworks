    ...
    while (e0) {
        ...
        do {
            ...
            if (t1.mMixerFormat != AUDIO_FORMAT_PCM_16_BIT) {
                ...
                applyLimiter(state, framecount, t1.mMixerInFormat, t1.mMixerChannelCount,
                    t1.mDevSampleRate, (int32_t*)outTemp, trackCount);
            }
#endif
...
{
#ifdef MTK_AUDIOMIXER_ENABLE_LIMITER
    if (!kUseFloat || !mLimiterEnable) {
        // Limiter only support floating AudioMixer
        return;
    }
    ...
    if (trackCount > 1 && state->mLimiter_status.State == LMTR_BYPASS_STATE) {
        ...
        ALOGV("limiter switch to LMTR_NORMAL_STATE");
        ...
        Limiter_SetParameters(state->mpLimiterObj, &param);
        ALOGV("limiter switch to LMTR_BYPASS_STATE");
    }
    ...
    MixerDumpPcm(gaf_mixer_limin_pcm_before, gaf_mixer_limin_propty, gettid(), 0, outTemp, inSize, DumpFormat, sampleRate, MixerChannelCount );
    ...
    MixerDumpPcm(gaf_mixer_limin_pcm_after, gaf_mixer_limin_propty, gettid(), 0, outTemp, outSize, DumpFormat, sampleRate, MixerChannelCount );
#endif
}
