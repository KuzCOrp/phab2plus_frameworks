...

{
    ...
    ///add begin
    int pos = Params.find("MIC_TYPE=");
    if (pos > -1) {
        char strVal[2];
        strncpy(strVal, Params.string()+pos+strlen("MIC_TYPE="), 1);
        strVal[1] = '\0';
        ALOGD("Params:%s, strVal:%c", Params.string(), strVal[0]);

    int value = strVal[0]-'0';
        switch(value) {
            case 1:
                flags = (audio_input_flags_t)AUDIO_INPUT_FLAG_TEST1;
                break;
            case 2:
                flags = (audio_input_flags_t)AUDIO_INPUT_FLAG_TEST2;
                break;
            case 3:
                flags = (audio_input_flags_t)AUDIO_INPUT_FLAG_TEST3;
                break;
			case 4:
				flags = (audio_input_flags_t)AUDIO_INPUT_FLAG_TEST4;
				break;
			case 5:
				flags = (audio_input_flags_t)AUDIO_INPUT_FLAG_TEST5;
				break;
			case 6:
				flags = (audio_input_flags_t)AUDIO_INPUT_FLAG_TEST6;
				break;
			case 10:
				flags = (audio_input_flags_t)AUDIO_INPUT_FLAG_TEST10;
				break;
            default:
                break;
        }
...
#ifdef MTK_AOSP_ENHANCEMENT
    // Assign correct channel mask for different audio source
    if (inputSource == AUDIO_SOURCE_VOICE_DOWNLINK) {
        channelMask = AUDIO_CHANNEL_IN_VOICE_DNLINK;
    } else if (inputSource == AUDIO_SOURCE_VOICE_UPLINK) {
        channelMask = AUDIO_CHANNEL_IN_VOICE_UPLINK;
    } else if (inputSource == AUDIO_SOURCE_VOICE_CALL) {
