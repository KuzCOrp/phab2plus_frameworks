...
OMX_ERRORTYPE SimpleSoftOMXComponent::freeBuffer(
    ...
    for (size_t i = 0; i < port->mBuffers.size(); ++i) {
        ...
        if (buffer->mHeader == header) {
            ...
            ALOGD("Free %s buffer (%p, id=%zu)", ((portIndex == 0) ? "input" : "output"), header, i);
#endif
            delete header;
            header = NULL;
...
#ifdef MTK_AOSP_ENHANCEMENT
        ALOGD("Transition from %d to %d %s",  mState, mTargetState, transitionComplete ? "complete" : "not complete");
#endif
