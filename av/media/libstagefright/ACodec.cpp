...
#ifdef MTK_AOSP_ENHANCEMENT
        // When mIsRecorder is set to 1, ACodec would not print per-frame log
        int32_t recorder;
...
status_t ACodec::setupAVCEncoderParameters(const sp<AMessage> &msg) {
    ...
    // To prevent the profile of AVC encoder from being forced to be baseline.
    if (!mIsRecorder && h264type.eProfile != OMX_VIDEO_AVCProfileBaseline) {
...
void ACodec::BaseState::onInputBufferFilled(const sp<AMessage> &msg) {
    ...
    */
    ...
    if (true == mCodec->mIsAudioBufferLogSwitchOn
        || (true == mCodec->mIsVideo && false == mCodec->mIsRecorder)
        || true == mCodec->mIsVideoBufferLogSwitchOn) {
...
#ifdef MTK_AOSP_ENHANCEMENT
    else if (true == mCodec->mIsNewSurface) {
        if( IsMTKVideoDecoderComponent(mCodec->mComponentName.c_str()) )
        {
            OMX_CONFIG_RECTTYPE rect;
            if (OK == mCodec->mOMX->getConfig(mCodec->mNode, OMX_IndexVendorMtkOmxVdecGetCropInfo,
                        &rect, sizeof(rect))) {
                ...
                status_t err = native_window_set_crop(mCodec->mNativeWindow.get(), &crop);
                ALOGW_IF(err != NO_ERROR, "failed to set crop for new surface: %d", err);
            }
            else {
                ALOGW("Failed to get crop info for new surface");
            }
        }
        mCodec->mIsNewSurface = false;
    }
#endif
    int32_t render;
    if (mCodec->mNativeWindow != NULL
